package com.wagerr.legacywallet.ui.base;
...
import com.wagerr.legacywallet.BuildConfig;
import com.wagerr.legacywallet.R;
import com.wagerr.legacywallet.module.WagerrAppContext;
import com.wagerr.legacywallet.ui.bet.event.BetEventActivity;
import com.wagerr.legacywallet.ui.contacts_activity.ContactsActivity;
import com.wagerr.legacywallet.ui.settings_activity.SettingsActivity;
import com.wagerr.legacywallet.ui.wallet_activity.WalletActivity;

import chain.BlockchainState;

import static com.wagerr.legacywallet.service.IntentsConstants.ACTION_NOTIFICATION;
import static com.wagerr.legacywallet.service.IntentsConstants.INTENT_BROADCAST_DATA_BLOCKCHAIN_STATE;
import static com.wagerr.legacywallet.service.IntentsConstants.INTENT_BROADCAST_DATA_PEER_CONNECTED;
import static com.wagerr.legacywallet.service.IntentsConstants.INTENT_BROADCAST_DATA_TYPE;
import static com.wagerr.legacywallet.service.IntentsConstants.INTENT_EXTRA_BLOCKCHAIN_STATE;

public class BaseDrawerActivity extends WagerrActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        if (lastBlockTime+ WagerrAppContext.OUT_OF_SYNC_TIME>now){
            ...
            if (peerHeight!=-1){
                if (wagerrModule.getChainHeight()+10>peerHeight) {
            ...
            if (wagerrModule.isAnyPeerConnected()) {
                long peerHeight = wagerrModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (wagerrModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
...
//        }
    ...
    protected double calculateBlockchainSyncProgress() {
        long nodeHeight = wagerrModule.getConnectedPeerHeight();
        if (nodeHeight>0){
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (wagerrModule.getChainHeight()*100) / nodeHeight;
        }
        return -1;
    }
