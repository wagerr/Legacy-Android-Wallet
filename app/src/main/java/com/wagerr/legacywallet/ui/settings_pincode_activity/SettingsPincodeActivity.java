package com.wagerr.legacywallet.ui.settings_pincode_activity;
    ...
    public void onKeyClicked(KeyboardFragment.KEYS key) {
        if (key.getValue()<10){
            ...
            if (lastPos==4){
                ...
                if (reason==OLDER_PIN){
                    if (pincode.equals(wagerrApplication.getAppConf().getPincode())){
                        clear();
                        reason = NEW_PIN;
                    ...
                    wagerrApplication.getAppConf().savePincode(pincode);
