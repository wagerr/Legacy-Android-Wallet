package com.wagerr.legacywallet.rate.db;
...
import com.wagerr.legacywallet.contacts.AbstractSqliteDb;
...
public class RateDb extends AbstractSqliteDb<WagerrRate> implements RateDbDao<WagerrRate> {
    ...
    @Override
    protected ContentValues buildContent(WagerrRate obj) {
    ...
    @Override
    protected WagerrRate buildFrom(Cursor cursor) {
        ...
        return new WagerrRate(coin,value,timestap);
    }
    ...
    public void insertOrUpdateIfExist(WagerrRate wagerrRate) {
        if (getRate(wagerrRate.getCode())==null){
            insert(wagerrRate);
        }else {
            updateByKey(KEY_COIN,wagerrRate.getCode(),wagerrRate);
        }
    }
}
