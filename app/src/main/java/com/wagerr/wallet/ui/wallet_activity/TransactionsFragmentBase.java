...
import static com.wagerr.wallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static com.wagerr.wallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static com.wagerr.wallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = wagerrModule.listTx();
    ...
    protected BaseRecyclerAdapter<TransactionWrapper, ? extends BaseRecyclerViewHolder> initAdapter() {
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (wagerrRate !=null) {
                    localCurrency = wagerrApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * wagerrRate.getRate().doubleValue()).movePointLeft(8)
                                    )
                                    + " " + wagerrRate.getCode();
                ...
                holder.title.setText(getAddressOrContact(wagerrModule,data));
